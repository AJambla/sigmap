...
description: SigMap generalizes across 21 repos, 32 languages, and multiple domains with 78.9% hit@5 in the latest saved v8.34.0 retrieval run.
head:
  - - meta
    - property: og:title
...
::: info Official v8.34.0 benchmark snapshot
...
these 105 tasks is yes — 78.9% hit@5 with no per-repo tuning in the latest saved v8.34.0 run.
:::
...
If you want one number to carry into launch messaging, use the shared `v8.34.0` snapshot rather than an older per-page variant:
