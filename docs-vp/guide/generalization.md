...
description: SigMap generalizes across 21 repos, 32 languages, and multiple domains with 78.9% hit@5 in the latest saved v8.33.0 retrieval run.
head:
  - - meta
    - property: og:title
      content: "SigMap Generalization — 78.9% hit@5 across 32 languages with R support"
  - - meta
...
::: info Official v8.33.0 benchmark snapshot
...
these 105 tasks is yes — 78.9% hit@5 with no per-repo tuning in the latest saved v8.33.0 run.
:::

- **21 repos** (including 3 R language repos)
- **32 languages** (added R and GDScript)
- **multiple domains**
- **78.9%** overall hit@5
- **no per-repo tuning**
...
If you want one number to carry into launch messaging, use the shared `v8.33.0` snapshot rather than an older per-page variant:
