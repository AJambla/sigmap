...
description: SigMap builds a deterministic, auditable signature-and-evidence map that AI agents, CI, and reviewers can trust and verify. Zero dependencies, no embeddings, fully offline. Proof — 78.6% hit@5, 43.4% fewer prompts, 96.6% average token reduction, 33 languages with R support.
head:
  - - meta
    - property: og:title
...
<div style="display:inline-flex;flex-wrap:wrap;gap:.5rem;justify-content:center;background:var(--vp-c-brand-soft,#ede9fe);border:1px solid rgba(124,106,247,.25);border-radius:999px;padding:.55rem .9rem;font-size:.9rem;color:var(--vp-c-text-1)">
  <span><strong>Release:</strong> v8.35.0</span>
  <span>·</span>
  <span><strong>New — the gate is now order-invariant, and the ceilings came off:</strong> graph-boost scores no longer depend on git history depth, so a shallow CI checkout and a developer clone score identically — the fix that made the retrieval baseline recordable again. The token budget now drops <code>src/test/**</code> before production code on JVM repos and protects application entry points. Extractor ceilings raised to Java parity (120 members · 200 signatures · 200 KB body scans), un-hiding 43–71% of member surface on Swift/PHP/Kotlin/Scala/C# repos. And the MCP server now passes @hasmcp/mcp-spec-test on both tested revisions — honest version negotiation plus session-less <code>server/discover</code>. Zero dependencies, offline, deterministic.</span>
</div>
<div style="margin-top:.4rem;display:inline-flex;flex-wrap:wrap;gap:.5rem;justify-content:center;background:var(--vp-c-default-soft,#f3f4f6);border:1px solid rgba(0,0,0,.08);border-radius:999px;padding:.55rem .9rem;font-size:.9rem;color:var(--vp-c-text-2)">
  <span><strong>Benchmark:</strong> sigmap-v8.35-main</span>
  <span>·</span>
...
Latest saved benchmark run: **2026-09-13 (v8.35.0)**.
