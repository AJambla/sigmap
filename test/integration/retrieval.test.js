...
// ---------------------------------------------------------------------------
test('MCP tools/list: returns 22 tools including query_context', () => {
  const [res] = mcpCall({ jsonrpc: '2.0', method: 'tools/list', id: 1 });
  assert.ok(res.result, 'should have result');
  assert.strictEqual(res.result.tools.length, 22, `expected 22 tools, got ${res.result.tools.length}`);
